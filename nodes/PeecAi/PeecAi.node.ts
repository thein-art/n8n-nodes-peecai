...
interface PeecApiResponse<T = unknown> {
	data: T;
}

interface PeecFilterParam {
	filterValues?: Array<{ field: string; operator: string; values: string }>;
}

interface PeecFilter {
	field: string;
	operator: string;
	values: string[];
}
		...
		properties: [
			// ------ Resource ------
			{
				...
				options: [
					...
					{ name: 'Search Query', value: 'searchQuery', description: 'Search queries generated by AI models' },
					{ name: 'Shopping Query', value: 'shoppingQuery', description: 'Shopping/product queries generated by AI models' },
			...
			// Search Query
			{
				...
				type: 'options',
				noDataExpression: true,
				displayOptions: { show: { resource: ['searchQuery'] } },
				options: [
					{ name: 'Get', value: 'get', action: 'Get search queries' },
				],
				default: 'get',
			},
			...
			// Shopping Query
			{
				...
				type: 'options',
				noDataExpression: true,
				displayOptions: { show: { resource: ['shoppingQuery'] } },
				options: [
					{ name: 'Get', value: 'get', action: 'Get shopping queries' },
				],
				default: 'get',
			},
			...
			// Project ID — required for all resources except project
			{
				...
				displayOptions: {
					show: {
						resource: ['brand', 'prompt', 'tag', 'topic', 'model', 'chat', 'brandReport', 'domainReport', 'urlReport', 'searchQuery', 'shoppingQuery'],
					},
				},
			},
			...
			// Limit/Offset for query resources (POST-based, no returnAll)
			{
				displayName: 'Limit',
				name: 'queryLimit',
				type: 'number',
				...
				description: 'Max number of results to return',
				displayOptions: {
					show: { resource: ['searchQuery', 'shoppingQuery'], operation: ['get'] },
				},
			},
			{
				displayName: 'Offset',
			...
			// Date filters — for chats and reports
			{
				...
				displayOptions: {
					show: {
						resource: ['chat', 'brandReport', 'domainReport', 'urlReport', 'searchQuery', 'shoppingQuery'],
					},
			...
			// Dimensions — for reports
			{
				...
				displayOptions: {
					show: {
						resource: ['brandReport', 'domainReport', 'urlReport', 'searchQuery', 'shoppingQuery'],
					},
				},
			},
	...
	methods = {
		loadOptions: {
			async getProjects(this: ILoadOptionsFunctions): Promise<INodePropertyOptions[]> {
				...
				const projects = (response as PeecApiResponse<Array<{ id: string; name: string }>>).data;
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				if (resource === 'project' && operation === 'getAll') {
					...
					applyDateParams(this, i, body);
					applyFilters(this, i, body);
				...
				else if ((resource === 'searchQuery' || resource === 'shoppingQuery') && operation === 'get') {
					const projectId = this.getNodeParameter('projectId', i) as string;
					const body: IDataObject = { project_id: projectId };

					applyDateParams(this, i, body);

					const limit = this.getNodeParameter('queryLimit', i) as number;
					const offset = this.getNodeParameter('queryOffset', i) as number;
					body.limit = limit;
					body.offset = offset;

					applyFilters(this, i, body);

					const endpoint = resource === 'searchQuery' ? '/queries/search' : '/queries/shopping';
					responseData = await apiRequestPost.call(this, endpoint, body);
				}
...
/** Parse date params and apply to target object. */
function applyDateParams(ctx: IExecuteFunctions, i: number, target: IDataObject): void {
	const startDate = ctx.getNodeParameter('startDate', i) as string;
	const endDate = ctx.getNodeParameter('endDate', i) as string;
	if (startDate) target.start_date = startDate.substring(0, 10);
	if (endDate) target.end_date = endDate.substring(0, 10);
}

/** Parse filter params and apply to target object. */
function applyFilters(ctx: IExecuteFunctions, i: number, target: IDataObject): void {
	const filtersParam = ctx.getNodeParameter('filters', i) as PeecFilterParam;
	if (filtersParam.filterValues && filtersParam.filterValues.length > 0) {
		target.filters = filtersParam.filterValues.map((f): PeecFilter => ({
			field: f.field,
			operator: f.operator,
...
async function apiRequest(
	...
	return (response as PeecApiResponse).data;
}
